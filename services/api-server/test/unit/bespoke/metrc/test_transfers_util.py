						...
						'json': [
							{
								...
								# This one needs to be the newest LastModified to get
								# past our check of only looking at newest transfers
								'LastModified': parser.parse('02/03/2020 12:00').isoformat(),
								# Incoming frields
								'DeliveryId': 'out-d1',
						...
						'json': [
							{
								...
								# This one needs to be the newest LastModified to get
								# past our check of only looking at newest transfers
								'LastModified': parser.parse('02/05/2020 12:00').isoformat()
							}
						]
					},
				]
...
class TestPopulateTransfersTable(db_unittest.TestCase):
							...
							'json': [
								{
									...
									'RecipientFacilityName': 'facility-name-recip1',
									'ReceivedDateTime': parser.parse('01/02/2020').isoformat(),
									'LastModified': parser.parse('02/05/2020 08:00').isoformat()
								}
							]
						}
					]
		...
		expected_transfer_packages: List[Dict] = [
			{
				...
				'delivery_row_id': metrc_delivery_row_ids[1],
				'transfer_row_id': transfer_row_ids[1],
				'last_modified_at': parser.parse('02/03/2020 12:00'),
			},
			{
				'type': 'transfer',
				'company_id': company_id,
			...
			},
			{
				...
				'delivery_row_id': metrc_delivery_row_ids[0],
				'transfer_row_id': transfer_row_ids[0],
				'last_modified_at': parser.parse('02/05/2020 12:00'),
			},
			{
				'type': 'transfer',
				'package_id': 'in-pkg2-B',
	...
	def test_populate_incoming_outgoing_transfers_all_internal(self) -> None:
		self._run_test({
			'index': 0, # this means we are doing all internal transfers
			'test_type': 'INTERNAL',
		})

	def test_populate_incoming_outgoing_transfers_all_two_companies(self) -> None:
		self._run_test({
