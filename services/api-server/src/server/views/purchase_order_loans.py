...
class SubmitForApprovalView(MethodView):
    ...
    def post(self) -> Response:
        ...
        with session_scope(current_app.session_maker) as session:
            ...
            if not purchase_order_loan.origination_date:
                return make_error_response('Invalid origination date')

            if purchase_order_loan.amount is None or purchase_order_loan.amount <= 0:
            ...
            for sibling_purchase_order_loan in sibling_purchase_order_loans:
                if sibling_purchase_order_loan.status in [RequestStatusEnum.ApprovalRequested, RequestStatusEnum.Approved]:
                    proposed_loans_total_amount += float(
                        sibling_purchase_order_loan.amount) if sibling_purchase_order_loan.amount else 0

            proposed_loans_total_amount += float(purchase_order_loan.amount)
            ...
            purchase_order_loan.status = RequestStatusEnum.ApprovalRequested
            purchase_order_loan.requested_at = date_util.now()
