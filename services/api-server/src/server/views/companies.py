...
class UpdateDummyAccountSettingsView(MethodView):
	decorators = [auth_util.bank_admin_required]
	...
	def post(self, **kwargs: Any) -> Response:
		...
		with session_scope(current_app.session_maker) as session:
			_, err = create_company_util.edit_dummy_account_status(
				session = session,
				parent_company_id = form['parent_company_id'],
				is_dummy_account = form['is_dummy_account'],
			)
			if err:
				raise err
...
class EditChildCompanyView(MethodView):
	decorators = [auth_util.login_required]
	...
	def post(self, **kwargs: Any) -> Response:
		...
		with session_scope(current_app.session_maker) as session:
			...
			_, err = create_company_util.edit_child_company(
				req=form,
				session=session,
			)
			if err:
...
handler.add_url_rule(
	'/edit_child_company', view_func=EditChildCompanyView.as_view(name='edit_child_company_view'))
