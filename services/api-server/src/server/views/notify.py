...
from typing import cast, Dict, List, Callable, Tuple
...
class NotifyHelper(object):
	...
	def __init__(self, 
		session_maker: Callable, type_config: TypeConfigDict, input_data: Dict) -> None:
	...
	def _get_company_info(self, company_id: str) -> CompanyInfoDict:
		...
		with session_scope(self._session_maker) as session:
			...
			settings = CompanySettingsDict(vendor_agreement_docusign_template='')
			if company_settings:
			...
			if company_users:
				recipients = [_user_to_recipient(user) for user in company_users]
	...
	def _get_vendor_info(self, vendor_id: str) -> VendorInfoDict:
		...
		with session_scope(self._session_maker) as session:
			...
			if vendor_users:
				recipients = [_user_to_recipient(user) for user in vendor_users]
	...
	def fetch(self) -> Tuple[List[SendPayload], str]:
		...
		if 'company_id' in self._input_data:
			company_info = self._get_company_info(self._input_data['company_id'])
...
class SendNotificationView(MethodView):
	...
	def post(self) -> Response:
		...
		helper = NotifyHelper(current_app.session_maker, form['type_config'], form['input_data'])
		...
		for send_payload in send_payloads:
			...
			_, err = sendgrid_client.send(template_name, template_data, recipient_emails)
			if err:
				return make_error_response(err)
