...
class ReportsLoansComingDueView(MethodView):
	...
	def filter_out_customers_opted_into_autogenerated_repayments(
		self,
		session : Session, 
		loans_to_notify : Dict[str, List[models.Loan] ],
	) -> Tuple[ Dict[str, List[models.Loan]], errors.Error ]:
		all_company_ids: List[str] = []
		for company_id in loans_to_notify:
			all_company_ids.append(company_id)

		company_settings = cast(
			List[models.CompanySettings],
			session.query(models.CompanySettings).filter(
				models.CompanySettings.company_id.in_(all_company_ids)
			).all())

		company_ids_to_filter_out: List[str] = []
		for settings in company_settings:
			customer_opt_in = settings.is_autogenerate_repayments_enabled

			if customer_opt_in:
				company_ids_to_filter_out.append(str(settings.company_id))

		for company_id in company_ids_to_filter_out:
			loans_to_notify.pop(company_id, None)
	...
	def process_loan_chunk(
		...
		loans_to_notify, err = self.filter_out_customers_opted_into_autogenerated_repayments(
			session,
			loans_to_notify,
		)
		if err:
			return loans_to_notify, make_response(json.dumps({ 'status': 'FAILED', 'resp': "Failed to filter out opt-in customers"}))
