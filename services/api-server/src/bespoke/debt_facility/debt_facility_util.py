...
def _check_if_status_change_moves_loans_to_update_required(
	...
	good_to_bad_status_check = (old_debt_facility_status == CompanyDebtFacilityStatus.GOOD_STANDING or \
		  old_debt_facility_status == CompanyDebtFacilityStatus.ON_PROBATION or \
		  old_debt_facility_status == CompanyDebtFacilityStatus.INELIGIBLE_FOR_FACILITY) and \
		 (new_debt_facility_status == CompanyDebtFacilityStatus.OUT_OF_COMPLIANCE or \
		  new_debt_facility_status == CompanyDebtFacilityStatus.DEFAULTING)

	bad_to_bad_status_check = (old_debt_facility_status == CompanyDebtFacilityStatus.OUT_OF_COMPLIANCE or \
		  old_debt_facility_status == CompanyDebtFacilityStatus.DEFAULTING or \
		  old_debt_facility_status == CompanyDebtFacilityStatus.INELIGIBLE_FOR_FACILITY) and \
		 (new_debt_facility_status == CompanyDebtFacilityStatus.OUT_OF_COMPLIANCE or \
		  new_debt_facility_status == CompanyDebtFacilityStatus.DEFAULTING) and \
...
def update_company_debt_facility_status(
	...
	if contract.product_type == ProductType.DISPENSARY_FINANCING and \
		...
		raise errors.Error('Cannot set dispensary finacing clients to good standing as \
	...
	if (old_debt_facility_status == CompanyDebtFacilityStatus.WAIVER or \
		old_debt_facility_status == CompanyDebtFacilityStatus.OUT_OF_COMPLIANCE or \
		old_debt_facility_status == CompanyDebtFacilityStatus.DEFAULTING) and \
		...
		for loan_report in loan_reports:
			if (loan_report.debt_facility_status == CompanyDebtFacilityStatus.WAIVER or \
	...
	elif _check_if_status_change_moves_loans_to_update_required(old_debt_facility_status, new_debt_facility_status):
		company.debt_facility_waiver_date = None
		company.debt_facility_waiver_expiration_date = None

		for loan_report in loan_reports:
			if (loan_report.debt_facility_status == LoanDebtFacilityStatus.SOLD_INTO_DEBT_FACILITY or \
				loan_report.debt_facility_status == LoanDebtFacilityStatus.WAIVER or \
		...
		company.debt_facility_waiver_expiration_date = date_util.load_date_str(waiver_expiration_date)

		for loan_report in loan_reports:
			if (loan_report.debt_facility_status != LoanDebtFacilityStatus.BESPOKE_BALANCE_SHEET and \
				...
				loan_report.debt_facility_waiver_expiration_date = date_util.load_date_str(waiver_expiration_date)
...
def check_past_due_loans(
	...
	while not is_done:
		...
		for company in companies:
			has_company_waiver = (company.debt_facility_status == CompanyDebtFacilityStatus.WAIVER)
			...
			loans = cast(
				...
				session.query(models.Loan).filter(
					...
					models.Loan.loan_report_id != None
				).all())
			...
			for loans_chunk in chunker(loans, LOAN_BATCH_SIZE):
				logging.info(f'Checking loans for past due batch {batch_index} of {batches_count} on company {company.name}...')
				...
				for loan_report in loan_reports:
					...
					has_loan_waiver = (loan_report.debt_facility_status == LoanDebtFacilityStatus.WAIVER)
