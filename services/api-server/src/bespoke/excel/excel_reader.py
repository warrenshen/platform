...
def _to_sheet_obj(workbook: Any, sheet_name: str, trim_empty_columns: bool) -> Tuple[Dict, str]:
	try:
		sheet = workbook[sheet_name]
	except Exception as e:
		return None, u'Reading from Excel error: {0}'.format(e)

	sheet_obj = {'rows': [], 'sheet_name': sheet_name}
	for row in sheet.iter_rows():
		...
		for cell in row:
			...
			text_row.append('{}'.format(cell.value).strip(''))
		sheet_obj['rows'].append(text_row)
...
class ExcelWorkbook(object):
	...
	def add_sheet(self, sheet_obj: Dict) -> None:
		self.d['sheets'].append(sheet_obj)

	def get_sheet_by_name(self, name: str) -> Tuple[Dict, str]:
	...
	def get_sheet_by_index(self, index: int) -> Tuple[Dict, str]:
