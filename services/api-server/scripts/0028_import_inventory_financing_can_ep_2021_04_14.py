...
def main() -> None:
	...
	purchase_orders_path = 'scripts/data/can_ep_purchase_orders_2021_04_14.xlsx'
	with models.session_scope(session_maker) as session:
		purchase_orders.load_into_db_from_excel(session, purchase_orders_path)

	loans_path = 'scripts/data/can_ep_loans_2021_04_14.xlsx'
	with models.session_scope(session_maker) as session:
		loans.load_into_db_from_excel(session, loans_path)

	advances_path = 'scripts/data/can_ep_advances_2021_04_14.xlsx'
	with models.session_scope(session_maker) as session:
		advances.load_into_db_from_excel(session, advances_path)

	repayments_path = 'scripts/data/can_ep_repayments_2021_04_14.xlsx'
	with models.session_scope(session_maker) as session:
