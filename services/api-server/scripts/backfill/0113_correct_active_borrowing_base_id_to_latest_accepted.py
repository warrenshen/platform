...
def main(is_test_run: bool = True) -> None:
	...
	while not is_done:
		with models.session_scope(session_maker) as session:
			print(f'[Page {current_page + 1}] correcting active_borrowing_base_id to latest accepted application')
			...
			try:
				...
				for company in companies:
					...
					if str(company.id) in company_settings_lookup:
						...
						if borrowing_bases is not None and \
							len(borrowing_bases) > 0 and \
							str(borrowing_bases[0].id) != company_settings.active_borrowing_base_id:
							print(f'Correcting the active borrowing base for { company.name } to application with id: { str(borrowing_bases[0].id) }')
							company_settings.active_borrowing_base_id = borrowing_bases[0].id
