...
def main(is_test_run: bool = True) -> None:
	...
	while not is_done:
		with models.session_scope(session_maker) as session:
			print(f'[Page {current_page + 1}] Updating metrc_sales_receipts.company_id...')

			try:
				metrc_receipts_batch = cast(
			...
			if len(metrc_receipts_batch) <= 0:
				print('Got 0 metrc sales receipts back')
				is_done = True
				continue
			else:
				for metrc_receipt in metrc_receipts_batch:
					print(f'Updating metrc receipt {str(metrc_receipt.id)}... TEST_RUN={is_test_run}')
					if not is_test_run:
						metrc_receipt.company_id = ra_company_id
