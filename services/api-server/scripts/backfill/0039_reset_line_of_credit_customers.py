...
def reset_customers(
	...
	for index, customer_tuple in enumerate(customer_tuples):
		...
		line_of_credits = session.query(models.LineOfCredit).filter(
			models.LineOfCredit.company_id == customer.id
		).all()

		print(f'[{index + 1} of {customers_count}] Resetting {len(line_of_credits)} line of credits...')

		for line_of_credit in line_of_credits:
			session.delete(line_of_credit)
		session.flush()
