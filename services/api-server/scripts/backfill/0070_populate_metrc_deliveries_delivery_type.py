...
def main() -> None:
	...
	with models.session_scope(session_maker) as session:
		company_licenses = cast(
			...
			session.query(models.CompanyLicense).all())
			...
			if len(metrc_deliveries_batch) <= 0:
				...
				for metrc_delivery in metrc_deliveries_batch:
					...
					if not metrc_delivery.delivery_type:
						...
						shipper_company_id = str(metrc_transfer.vendor_id)

						transfer_type = None
						if shipper_company_id and shipper_company_id == company_id:
							transfer_type = db_constants.TransferType.OUTGOING
						elif recipient_company_id and recipient_company_id == company_id:
