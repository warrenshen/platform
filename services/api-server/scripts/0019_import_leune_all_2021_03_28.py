...
def main() -> None:
	...
	payor_vendors_path = 'scripts/data/payor_vendors_2021_03_28.xlsx'
	with models.session_scope(session_maker) as session:
		companies.load_into_db_from_excel(session, payor_vendors_path)

	purchase_orders_path = 'scripts/data/leune_purchase_orders_2021_03_28.xlsx'
	with models.session_scope(session_maker) as session:
		purchase_orders.load_into_db_from_excel(session, purchase_orders_path)

	loans_path = 'scripts/data/leune_loans_2021_03_28.xlsx'
	with models.session_scope(session_maker) as session:
		loans.load_into_db_from_excel(session, loans_path)

	advances_path = 'scripts/data/leune_advances_2021_03_28.xlsx'
	with models.session_scope(session_maker) as session:
		advances.load_into_db_from_excel(session, advances_path)

	repayments_path = 'scripts/data/leune_repayments_2021_03_28.xlsx'
	with models.session_scope(session_maker) as session:
